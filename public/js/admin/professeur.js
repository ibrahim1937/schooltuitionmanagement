$(document).ready(function() {
    ...
    $(document).on("change", "#profsearch", function() {
        if ($(this).val()) {
            $.ajax({
                url: gestionprofurl,
                type: "POST",
                data: {
                    _token: $(document)
                        ...
                        .attr("content"),
                    op: "filitrage",
                    id_prof: parseInt($(this).val())
                },
                dataType: "json",
                success: function(data) {
                    fillAllSearch(".display", $("#content-prof-search"), data);
                },
                error: function(error) {
                    console.log(error);
                }
...
function remplirsearch(selector, myData) {
    var ligne = "";
    ...
    for (let i = 0; i < myData.length; i++) {
        ligne +=
            ...
            "</th>";
        ligne += "<td> " + myData[i].nom + "</td>";
        ligne += "<td> " + myData[i].module.nom + "</td>";
        ligne += "<td> " + myData[i].professeur.nom + "</td>";
    }

    selector.html(ligne);
}

function fillAllSearch(table, selector, myData) {
    if ($.fn.DataTable.isDataTable(table)) {
        $(table)
            .DataTable()
            .destroy();
    }
    remplirsearch(selector, myData);
    $(table).DataTable({
        order: []
    });
}
